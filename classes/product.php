	...
	class product
	{
		...
		public function insert_product($date,$files){
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				...
				// đua ảnh từ bên ngoài vào image trong admin
				move_uploaded_file($file_temp, $uploaded_image);
				// lay anh uploads vao file_temp
		...
		public function update_product($data,$files,$id){
			...
			$div = explode('.', $file_name);// khi gap dau cham se phan tach
			$file_ext = strtolower(end($div));// vd: 123.jpg thi end lay jpg
			// $file_current = strtolower(current($div)); lay 123
			$unique_image = substr(md5(time()), 0, 10).'.'.$file_ext;
			$uploaded_image = "uploads/".$unique_image;
			...
			if($product_code == "" || $productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""){
				...
				if(!empty($file_name)){
					...
					move_uploaded_file($file_temp,$uploaded_image); // di chuyen anh vao
					$query = "UPDATE tbl_product SET
		...
		public function getproduct_new()
		{
			...
			// lấy ra $sp_tungtrang=4 từ vị trí $tung_trang
		...
		// laays ra 1 sp dell moi nhat
		public function getLastestDell()
		{
			$query = "SELECT * FROM tbl_product where brandId = '10' order by productId desc limit 1";
			// brandId = '10'  vi trong CSDL dell co brandid la 10 va tuong tu vs cac hang khac
			$result = $this->db->select($query);
			return $result;	
		}
		...
		public function insertCompare($productid, $customer_id)
		{
			...
			// phaang kieam tra xem sp da co trong bang so sanh hay chua
			...
			// phần insert nếu san phẩm ko có trong bảng so sánh
